...
if tree.check_monophyly(values=eukaryote_seqs, target_attr="name"):
    print "Euks monophyletic"
else:
    print "Euks not monophyletic" #this could, in itself, be a possible criterion for LGT --- or gene loss/transfer. Perhaps a condition that the target gene is not in the largest eukaryotic sub-clade (get_monophyletic() method will return subclades with a particular attribute that are monophyletic)
