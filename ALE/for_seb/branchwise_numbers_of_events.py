#read in a set of ALEml_undated reconciliation files and sum up the number of events on each branch, across the dataset
import os, sys, re
from collections import defaultdict

branchwise = defaultdict(list) #key = branch number (from uml_rec); list 0th element: duplications, 1st element: transfers, second element: losses, third: originations, fourth: gene copy number --- if uml_rec output from ALE is modified to output more stuff, e.g. PPs, the mapping of these elements will change. 
